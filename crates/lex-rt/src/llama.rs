...
//!
//! Every layer op is a typed `lex-front` program, checked against the target
//! and lowered to MSL by `lex-msl` — the same path flash decode takes:
//!
//! ```text
...
mod gpu {
    ...
    impl Default for Attention {
        ...
        /// combined, 0.78 serial), K/V read directly. `LEX_BPS`,
        /// `LEX_MIN_SPLITS`, `LEX_ATTN_BK` and `LEX_KV_DIRECT=0` override
        /// it for tuning.
        fn default() -> Attention {
            ...
            Attention {
                bps: env("LEX_BPS", 2),
                min_splits: env("LEX_MIN_SPLITS", 2),
                bk: env("LEX_ATTN_BK", 16),
                kv_direct: env("LEX_KV_DIRECT", 1) != 0,
            }
        }
    }
    ...
    impl<'w> Runner<'w> {
        ...
        pub fn with_attention(w: &'w Weights, attention: Attention) -> Result<Runner<'w>, String> {
            ...
            let fold = (
                tune("LEX_FOLD_QKV", 0) != 0,
                tune("LEX_FOLD_GLU", 0) != 0,
                tune("LEX_FOLD_HEAD", 0) != 0,
            );
