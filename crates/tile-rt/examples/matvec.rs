...
fn main() -> Result<(), String> {
    ...
    println!(
        "  {:<22} {:>6} {:>4} {:>9} {:>10} {:>9}",
        "gate/up, batched", "tokens", "bo", "us", "GB/s", "ms/token"
    );
    let (n_in, n_out) = (5120usize, 17408usize);
    for tokens in [1usize, 2, 4] {
        for bo in [4usize, 8, 16, 32] {
            let prog = match matmul_q(tokens, n_in, n_out, bo, n_in, QLayout::NVFP4, false) {
                Ok(p) => p,
                Err(_) => continue,
            };
            ...
            let best = (0..3)
                ...
                / reps as f64;
            println!(
                "  {:<22} {tokens:>6} {bo:>4} {:>9.1} {:>10.0} {:>9.2}",
                "",
                best * 1e6,
                wbytes as f64 / best / 1e9,
                14.5e9 / (wbytes as f64 / best) * 1e3 / tokens as f64
            );
        }
    }
    Ok(())
