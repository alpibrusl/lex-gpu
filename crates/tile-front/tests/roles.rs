...
use tile_front::interp::{RunOptions, run_with};
use tile_front::ir::{Arg, Builder, IdxExpr, Op, RoleDef, TileTy, Ty, Var, View};
use tile_front::{Kind, Program, Tensor, check, run};
...
fn consumer_handle_given_to_two_roles() {
    ...
    let ty = pipe.ty.clone();
    let consumer = |ty: tile_front::ir::PipeTy| RoleDef {
        name: "consumer",
        warps: 1,
