//! `--flash`: flash-attention decode, a typed tile-front kernel lowered to MSL.
//!
...
use tile_msl::program::{Lowered, lower};
...
pub fn lowered(args: &Args, target: &Target) -> Result<(FlashDecode, Lowered), String> {
    let cfg = config(args);
    let prog = cfg.build()?;
    tile_front::check(&prog, target).map_err(|errs| {
        errs.iter()
            .map(|e| e.to_string())
            .collect::<Vec<_>>()
            .join("\n")
...
pub fn run(args: &Args, target: &Target) -> std::process::ExitCode {
    ...
    let gpu = match tile_metal::Gpu::open() {
        Ok(g) => g,
        Err(e) => {
            eprintln!("error: {e}");
            return ExitCode::FAILURE;
    ...
    for head in [0, h - 1] {
        ...
        let want = tile_front::flash::reference(
    ...
    let copy = tile_ir::Kernel::copy(DType::F16, args.copy_elems());
    let copy_s = match tile_ir::plan(&copy, target)
