...
#[cfg(target_os = "macos")]
use lex_ir::DType;
use lex_ir::{Kernel, Target, plan, reference};
...
fn main() -> ExitCode {
    ...
    if args.emit {
        for k in [&copy, &norm] {
            match plan(k, &target) {
                Ok(p) => println!("{}", lex_msl::emit(k, &p, &target)),
                Err(e) => {
...
fn run_device(_args: &Args, _target: &Target) -> ExitCode {
    ...
    println!("Run `cargo run --release -p lex-bench` on the Mac Studio for the numbers,");
...
#[cfg(target_os = "macos")]
fn verify_f32(gpu: &lex_metal::Gpu, args: &Args, target: &Target) -> Result<Accuracy, String> {
...
#[cfg(target_os = "macos")]
fn verify_f16(gpu: &lex_metal::Gpu, args: &Args, target: &Target) -> Result<Accuracy, String> {
    use half::f16;
...
#[cfg(target_os = "macos")]
fn check_execution_width(pipeline: &lex_metal::Pipeline, target: &Target) {
...
#[cfg(target_os = "macos")]
fn measure(gpu: &lex_metal::Gpu, args: &Args, target: &Target) -> Result<Vec<Measurement>, String> {
    let copy = Kernel::copy(args.dtype, args.copy_elems());
    let norm = Kernel::rmsnorm(args.dtype, args.rows, args.cols, args.eps);
...
#[cfg(target_os = "macos")]
fn filled(gpu: &lex_metal::Gpu, dtype: DType, len: usize, seed: u32) -> lex_metal::Buffer {
    match dtype {
        DType::F32 => {
