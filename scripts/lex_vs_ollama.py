"""Run the same prompts through lex (on the GPU) and through Ollama, and
compare them token by token.

    python3 scripts/lex_vs_ollama.py
    python3 scripts/lex_vs_ollama.py --prompt "Why is the sky blue?" --steps 32

For every prompt: tokenise it the way Ollama does (Llama 3 BPE plus BOS,
checked in `llama_ref.py`), greedy-decode with lex's `generate` example,
ask Ollama for the same greedy continuation with top-k log-probabilities, and
compare:

- the generated tokens must be identical;
- for every token in Ollama's top-k at every step, lex's log-probability
...
32 layers). Against an f32 PyTorch reference, lex is within 0.007 on both:
the gap is llama.cpp's rounding, not lex's. A real bug is far larger. A
missing BOS token, for example, shifted log-probabilities by 1.9 while every
token still matched.

    python3 scripts/lex_vs_ollama.py --model llama3.1:8b    # P2's exit model
"""
...
def lex(model, ids, steps, top):
    cmd = [
        "cargo", "run", "-q", "--release", "-p", "lex-rt", "--example", "generate", "--",
        "--model", model, "--ids", ",".join(map(str, ids)), "--steps", str(steps), "--top", str(top),
    ]
    p = subprocess.run(cmd, cwd=ROOT, capture_output=True, text=True)
    if p.returncode != 0:
        sys.exit(f"lex generate failed:\n{p.stderr}")
    out, timing = [], {}
...
def main():
    ...
    for prompt in prompts:
        ...
        mine, timing = lex(a.model, ids, a.steps, a.top)
        ...
        if diverged is not None:
            print(f"  diverged at step {diverged}: lex {tk.decode([mine[diverged][0]])!r}, "
                  f"ollama {ref[diverged]['token']!r}")
        print(f"  lex decode {timing.get('decode_tok_s', 0):.1f} tok/s on the GPU")
